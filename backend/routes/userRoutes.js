...
router.post('/logout', async (req, res) => {
    try {
      // Ambil user dari token atau sesi
      const userId = req.user._id; // atau cara lain untuk mendapatkan userId
  
      // Temukan user dan update logoutTimestamp
      const user = await User.findById(userId);
      if (!user) {
        return res.status(404).json({ error: 'User not found' });
      }
  
      user.logoutTimestamp = new Date(); // Set logoutTimestamp
      await user.save();
